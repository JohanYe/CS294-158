...
ax[0].set_ylabel('ELBO in bits per dim')
...
ax[1].set_ylabel('ELBO in bits per dim')
...
ax[1, 0].set_title("Vector No decoder noise")
ax[1, 0].set_xlim(-17.5, 17.5)
ax[1, 0].set_ylim(-17.5, 17.5)
ax[1, 0].legend(loc='best')
...
ax[1, 1].scatter(X_val.cpu()[:1000, 0], X_val.cpu()[:1000, 1], label='X original')
ax[1, 1].scatter(reconstructions[:, 0], reconstructions[:, 1], label='Recon with noise')
ax[1, 1].set_title("Vector Decoder noise")
ax[1, 1].set_xlim(-17.5, 17.5)
ax[1, 1].set_ylim(-17.5, 17.5)
ax[1, 1].legend(loc='best')
...
ax[0, 0].set_title("Scalar No decoder noise")
ax[0, 0].set_xlim(-17.5, 17.5)
ax[0, 0].set_ylim(-17.5, 17.5)
ax[0, 0].legend(loc='best')
...
ax[0, 1].scatter(X_val.cpu()[:1000, 0], X_val.cpu()[:1000, 1], label='X original')
ax[0, 1].scatter(reconstructions_val[:, 0], reconstructions_val[:, 1], label='Recon with noise')
ax[0, 1].set_title("Scalar Decoder noise")
ax[0, 1].set_xlim(-17.5, 17.5)
ax[0, 1].set_ylim(-17.5, 17.5)
ax[0, 1].legend(loc='best')
if ds1:
...
ax[0].scatter(X_val.cpu()[:1000, 0], X_val.cpu()[:1000, 1], c=y, label='Original data colored')
ax[0].set_title('Original data, colored')

ax[2].scatter(prior[:, 0], prior[:, 1], label='prior')
ax[2].scatter(latent[:, 0], latent[:, 1], c=y, label='latent_z')
ax[2].scatter(mu_z[:, 0], mu_z[:, 1], color='green', label='mean')
ax[2].legend(loc='best')
ax[2].set_title('Latent visualization of Vector Variance Network')
...
ax[1].scatter(mu_z2[:, 0], mu_z2[:, 1], color='green', label='mean')
ax[1].legend(loc='best')
ax[1].set_title('Latent visualization of Scalar Variance Network')
fig.tight_layout()
plt.savefig('./Hw3/Figures/latent_visualization.pdf', bbox_inches='tight')

fig, ax = plt.subplots(2, 2, figsize=(10, 10))

reconstructions = net_scalar(X_val[:1000], noise=False).detach().cpu().numpy()
ax[0, 0].scatter(reconstructions[:, 0], reconstructions[:, 1], c=y, label='Recon without noise')
ax[0, 0].set_title("Reconstructions Scalar No decoder noise")
ax[0, 0].set_xlim(-17.5, 17.5)
ax[0, 0].set_ylim(-17.5, 17.5)
ax[0, 0].legend(loc='best')

reconstructions = net_scalar(X_val[:1000], noise=True).detach().cpu().numpy()
ax[0, 1].scatter(reconstructions[:, 0], reconstructions[:, 1], c=y, label='Recon without noise')
ax[0, 1].set_title("Reconstructions Scalar decoder noise")
ax[0, 1].set_xlim(-17.5, 17.5)
ax[0, 1].set_ylim(-17.5, 17.5)
ax[0, 1].legend(loc='best')

reconstructions = net_vector(X_val[:1000], noise=False).detach().cpu().numpy()
ax[1, 0].scatter(reconstructions[:, 0], reconstructions[:, 1], c=y, label='Recon without noise')
ax[1, 0].set_title("Reconstructions Vector No decoder noise")
ax[1, 0].set_xlim(-17.5, 17.5)
ax[1, 0].set_ylim(-17.5, 17.5)
ax[1, 0].legend(loc='best')

reconstructions = net_vector(X_val[:1000], noise=True).detach().cpu().numpy()
ax[1, 1].scatter(X_val.cpu()[:1000, 0], X_val.cpu()[:1000, 1], c=y, label='X original')
ax[1, 1].scatter(reconstructions[:, 0], reconstructions[:, 1], c=y, label='Recon with noise')
ax[1, 1].set_title("Reconstructions Vector Decoder noise")
ax[1, 1].set_xlim(-17.5, 17.5)
ax[1, 1].set_ylim(-17.5, 17.5)
ax[1, 1].legend(loc='best')
plt.savefig('./Hw3/Figures/fig_latent.pdf', bbox_inches='tight')
